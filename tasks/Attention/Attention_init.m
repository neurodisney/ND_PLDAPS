...
p.defaultParameters.task.ProbHemi = 1; % if 1, randomly pick left or right hemifield
p.defaultParameters.task.RandomPar  = 0; % if 1, randomly change orientation and spatial frequency of the grating each trial
p.defaultParameters.task.AttProb = ['y'; 'y';'y';'y';'y';'y';'y';'y';'y';'n'];
...
p.defaultParameters.stim.OriLst     = 0:15:360;  % orientation of grating
p.defaultParameters.stim.Ref.sFreq  = datasample(p.defaultParameters.stim.sFreqLst,1); % spatial frequency as cycles per degree
p.defaultParameters.stim.AttQuad = 'ur';
p.defaultParameters.stim.QuadLst = ['ul'; 'll'; 'lr'];
