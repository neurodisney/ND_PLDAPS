...
p.defaultParameters.stim.PosY = 0; %  to get hardcoded single position
p.defaultParameters.stim.PosX = 0; % to get hardcoded single position

% define grid locations used by key selection
p.defaultParameters.stim.EccLst = [0, 0, 0, 0, 0, 0, 0, 0, 0]; % if p.defaultParameters.task.RandomEcc = 0; these are the eccentricities
p.defaultParameters.stim.AngLst = [0, 0, 0, 0, 0, 0, 0, 0, 0]; % if p.defaultParameters.task.RandomAng  = 0; these are the angles
...
p.defaultParameters.stim.sFreqLst   = [0.15, 0.15, 0.15]; % spatial frequency as cycles per degree
p.defaultParameters.stim.OriLst     = [0, 0, 0, 0, 0, 0, 0, 0, 0];  % orientation of grating
