...
RevCorr_Behav = function(datadir=NA, fname=NA) {
...
if(interactive()) {
  x11(width=19.5, height=10.5, pointsize=10, title='RevCorr_Behav')
} else {
  # Otherwise only save the figure as a pdf.
  pdf('RevCorr.pdf', 19.5, 10.5, pointsize=10, title='RevCorr_Behav')
}
...
if(interactive()) {
  ...
  dev.copy(pdf, 'RevCorr.pdf', 19.5, 10.5, pointsize=10, title='RevCorr_Behav')
}
