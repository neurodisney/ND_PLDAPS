...
switch p.trial.stim.RFmeth
    case 'coarse'
        ...
        p.trial.stim.coarse.yRange   = [-6, -1];
...
p.trial.reward.Dur          = .1; % reward duration for pulse in reward series while keeping fixation
p.trial.reward.Step         = [0, 6, 12, 18 24];     % define the number of subsequent rewards after that the next delay period should be used.
p.trial.reward.Period       = [0.5 .5 .5 .5 .5]; % the period between one reward and the next NEEDS TO BE GREATER THAN Dur
...
% Main trial timings
p.trial.task.CurRewDelay    = 0.1;  % Time to first reward
p.trial.task.fixLatency     = 0.15;  % Time to hold fixation before mapping begins
p.trial.task.jackpotTime    = 4.0;   % How long stimuli are presented before trial ends and jackpot is given
...
p.trial.task.Timing.MaxITI  = 3.5;  % maximum time period [s] between subsequent trials
