...
switch p.trial.stim.RFmeth
    case 'coarse'
        ...
        p.trial.stim.coarse.radius   = 0.6;      % size of grating 
        ...
        p.trial.stim.coarse.sFreq    = 2.0;       % spatial frequency
        ...
        p.trial.stim.coarse.xRange   = [0.0, 2.0];
        p.trial.stim.coarse.yRange   = [-2.0, -0.5];
...
p.trial.stim.OffTime = 0.15;   % these are overwritten, don't use to change times - use p.trial.task below, but make sure matches here for stim.period for file output
...
p.trial.reward.IncrConsecutive = 0; % Increases jackpot according to number of correct trials done in a row (1 = on, 0 = off)
...
p.trial.reward.IncrProgressive = 0; % Increases jackpot as more and more trials completed, regardless of performance
...
p.trial.task.jackpotTime    = 2.00;   % How long stimuli are presented before trial ends and jackpot is given
...
p.trial.stim.fixSpotParameters.colorList = {'dRed', 'green', 'blue'}; % color of fixation spot (as defined in the lookup tables)
p.trial.stim.fixSpotParameters.color = 'dRed';
