...
RFmap_Behav = function(datadir=NA, fname=NA) {
...
if(interactive()) {
  x11(width=19.5, height=10.5, pointsize=10, title='RFmap_Behav')
} else {
  # Otherwise only save the figure as a pdf.
  pdf(paste('RFmap_',dt$Date[1],'.pdf',sep=""), 19.5, 10.5, pointsize=10, title='RFmap_Behav')
}
...
if(interactive()) {
  ...
  dev.copy(pdf, 'RFmap.pdf', 19.5, 10.5, pointsize=10, title='RFmap_Behav')
}
