...
% Main trial timings
p.trial.task.fixLatency       = 0.25; % Time to hold fixation before initial reward is given
p.trial.task.stimLatency      = ND_GetITI(0.25, 0.35); % Time from initial reward to stim appearing 
...
p.trial.stim.grating.sFreq        = 3.25; % spatial frequency as cycles per degree, suggested range (WZ): 1-10 cycles/degree

p.trial.stim.grating.lowContrast  = 0.25;  % grating contrast value when stim.on = 1
p.trial.stim.grating.highContrast = 0.65;  % grating contrast value when stim.on = 2

orientations               = [30];   %  angles for the stim
