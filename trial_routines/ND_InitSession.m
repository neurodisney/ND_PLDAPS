...
if(p.defaultParameters.pldaps.draw.joystick.use && p.defaultParameters.datapixx.useJoystick)
    ...
    p.defaultParameters.pldaps.draw.joystick.levelsz =  [1,1]; % changing the joystick pointer size, will need to make a switch condition for this depending on if we want to keep the meter stuff for the future - MJH
