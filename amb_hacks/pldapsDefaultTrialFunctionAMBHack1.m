function pldapsDefaultTrialFunctionAMBHack1(p,state)
% created xx/xx/16 AMB
% last updated xx/xx/16 AMB
%
% list of subfunctions:
%  frameUpdate
%   - query kb
%   - mouse
%   - hacked EM sample -- fix this
%   - neuroDisney epoch checker
%   - pds.datapixx.adc.getData(p)
%   - pds.eyelink.getQueue(p)
%  frameDraw
%   - check if bg color changed, redraw if so
%   - check if grid is used, draw if so
%   - ND: draw stimulus
%   - framerate checker routine
%   - draw eyepos
%   - draw mouse
%   - draw photodiode
%  frameIdlePreLastDraw
%   - function was empty/commented out, so deleted (11/28/16 amb)
%  frameDrawingFinished
%   - just calls Screen('DrawingFinished', p.trial.display.ptr,0,0);
%  frameIdlePostDraw
%   - function was empty/commented out, so deleted (11/28/16 amb)
%  frameFlip
%   - flip frame, log time
%   - add to movie if being made
%   - check + draw bg, overlay again [maybe remove this? also done in Draw]
%  trialSetup
%   -
%  trialPrepare
%   -
%  p.trial.pldaps.experimentAfterTrialsFunction
%   - initialize p.trial.timing.flipTimes, frameStateChangeTimes
%   - initialize photodiode stuff if used
%   - pds.datapixx.adc.trialSetup(p)
%
%disp([datestr(now) 'state=' num2str(state)])
switch state
    %frameStates
    ...
    case p.trial.pldaps.trialStates.frameFlip;
        ...
        %trialStatesa
    case p.trial.pldaps.trialStates.trialSetupNA
        trialSetup(p);
...
if p.trial.mouse.use
    ...
    if(p.trial.mouse.useAsEyePos)
        ...
        % moved to ET section proper
        %                 Datapixx('RegWrRd'); % Update register of PROPixx Controller
        %                 V = Datapixx('GetAdcVoltages'); % Collect voltage values at all ADCs
        %                 p.trial.eyeX = ((V(1)+10)/20)*p.trial.display.pWidth;
        %                 p.trial.eyeY = ((V(2)+10)/20)*p.trial.display.pHeight;
        % this is in -10:10 range; need to map to screenpos
...
% grab data alll the time
Datapixx('RegWrRd'); % Update register of PROPixx Controller
V = Datapixx('GetAdcVoltages'); % Collect voltage values at all ADCs
p.trial.eyeX = ((V(1)+10)/20).*p.trial.display.pWidth;
p.trial.eyeY = ((V(2)+10)/20).*p.trial.display.pHeight;
%display analog input eye pos - AMB WIP - move from mouse section
% if p.trial.AIeyetracker.draw
%     Datapixx('RegWrRd'); % Update register of PROPixx Controller
%     V = Datapixx('GetAdcVoltages'); % Collect vopldapsDefaultTrialFunctionAMBHack1ltage values at all ADCs
%     p.trial.eyeX = ((V(1)+10)/20)*p.trial.display.pWidth;
%     p.trial.eyeY = ((V(2)+10)/20)*p.trial.display.pHeight;
%     % then do stuff!
% end
...
pds.datapixx.adc.getData(p);
%% now check EPOCH during the frameUpdate, this check happens as fast as possible
p.trial.fixTrain.currT=GetSecs-p.trial.trstart;
if p.trial.fixTrain.epoch==1 % prestim period
    % check if timer has passed
    if (p.trial.fixTrain.currT-p.trial.fixTrain.preStimTOn)...
            >=p.trial.fixTrain.preStimTS
        % set to new epoch
        p.trial.fixTrain.epoch=2; % stim onset
        % do timer here or later?
        p.trial.fixTrain.acquireTOn=GetSecs-p.trial.trstart;
        disp([datestr(now,'HH:MM:SS.FFF') ...
            ' prestim period passsed, showing stim'])
    end
elseif p.trial.fixTrain.epoch==2 % stimulus is on
    % check if fixation has started
    if ~(p.trial.eyeX<p.trial.fixTrain.winEdgeMinX) && ...
            ~(p.trial.eyeX>p.trial.fixTrain.winEdgeMaxX) && ...
            ~(p.trial.eyeY<p.trial.fixTrain.winEdgeMinY) && ...
            ~(p.trial.eyeY>p.trial.fixTrain.winEdgeMaxY)
        % fix has started! change epoch flag and start timer
        p.trial.fixTrain.epoch=4;
        p.trial.fixTrain.fixTOn=GetSecs-p.trial.trstart;
        disp([datestr(now,'HH:MM:SS.FFF') ' fixation onset'])
        %         disp([datestr(now) ' X='])
        %         disp([datestr(now) ' Y='])
        % if not, check time
    elseif (p.trial.fixTrain.currT-p.trial.fixTrain.acquireTOn)...
            >=p.trial.fixTrain.acquireTS
        % takin' too long mfker
        % change state to stimAcqTimeOut
        p.trial.fixTrain.epoch=3;
        % time-out, and then end da fckin trial
        p.trial.fixTrain.failTOn=GetSecs-p.trial.trstart;
        % set le stimulus color to failColor
        p.trial.fixTrain.stimColorRGB=p.trial.fixTrain.punishColorRGB;
        p.trial.fixTrain.stimType='square';
        p.trial.fixTrain.stimSizeXYPx=p.trial.fixTrain.stimSizeXYPx.*.75;
        disp([datestr(now,'HH:MM:SS.FFF') ' took too to start fixate'])
    end
elseif p.trial.fixTrain.epoch==3 % shouldnt occur, didnt acquire fix
    % should we handle this here?
    if (p.trial.fixTrain.currT-p.trial.fixTrain.failTOn)...
            >=p.trial.fixTrain.failTS
        % set to next trial!
        %p.trial.state=-3; % how do i actually do thees
        %setTimeAndFrameState(p,-3);
        p.trial.flagNextTrial=1;
        disp([datestr(now,'HH:MM:SS.FFF') ' neverfixed timeout over. going to next trial'])
        p.trial.fixTrain.epoch=-1; % moving to next trial
    end
elseif p.trial.fixTrain.epoch==4 % means fix was acquired
    % check if eye pos in region
    % this all assumed calibration as well
    % rectangular regions only atm
    if (p.trial.eyeX<p.trial.fixTrain.winEdgeMinX) || ...
            (p.trial.eyeX>p.trial.fixTrain.winEdgeMaxX) || ...
            (p.trial.eyeY<p.trial.fixTrain.winEdgeMinY) || ...
            (p.trial.eyeY>p.trial.fixTrain.winEdgeMaxY)
        % mfcka u out of bounds!!!
        p.trial.fixTrain.epoch=5;
        % timer
        p.trial.fixTrain.failTOn=GetSecs-p.trial.trstart;
        % set le stimulus color to background
        p.trial.fixTrain.stimColorRGB=p.trial.fixTrain.bgColorRGB;
        p.trial.fixTrain.stimType='square';
        p.trial.fixTrain.stimSizeXYPx=p.trial.fixTrain.stimSizeXYPx.*.75;
        disp([datestr(now,'HH:MM:SS.FFF') ' fixation was broken!'])
        % ok, see if fixation duration has passed
    elseif (p.trial.fixTrain.currT-p.trial.fixTrain.fixTOn)...
            >=p.trial.fixTrain.holdTS
        % eyy u did it, u completed the trial!
        p.trial.fixTrain.epoch=6;
        p.trial.fixTrain.goodTOn=GetSecs-p.trial.trstart;
        p.trial.fixTrain.stimColorRGB=p.trial.fixTrain.rewardColorRGB;
        disp([datestr(now,'HH:MM:SS.FFF') ...
            ' fixation maintained successfully, start reward'])
    end
    % check time
elseif p.trial.fixTrain.epoch==5 % fixBorked
    if (p.trial.fixTrain.currT-p.trial.fixTrain.failTOn)...
            >=p.trial.fixTrain.failTS
        % set to next trial!
        %p.trial.state=-3; % how do i actually do thees
        p.trial.flagNextTrial=1;
        disp([datestr(now,'HH:MM:SS.FFF') ...
            ' fixbroken timeout over. moving to next trial'])
        p.trial.fixTrain.epoch=-1; % moving to next trial
    end
elseif p.trial.fixTrain.epoch==6 % fixDone % 6+7 merged for now
    % could implement another timer here
    % go to reward state
    %p.trial.fixTrain.epoch=7;
    %elseif p.trial.fixTrain.epoch==7 % rewardtiem
    % 'do reward'
    % e.g. change stimulus color
    % wait until timer to end trial
    if (p.trial.fixTrain.currT-p.trial.fixTrain.goodTOn)...
            >=p.trial.fixTrain.goodTS;
        p.trial.flagNextTrial=1;
        disp([datestr(now,'HH:MM:SS.FFF') ' reward period passsed'])
        p.trial.fixTrain.epoch=-1; % moving to next trial
    end
elseif p.trial.fixTrain.epoch==8 % iticomplete % dont really need
elseif p.trial.fixTrain.epoch==9 % itiabort
end
%%
...
if strcmp(p.trial.fixTrain.stimType,'circle')
    
    
    % try with FIllOval -- later
    % note max stimsize 189px right now...
    %     p.trial.display.overlayptr
    %     p.trial.fixTrain.stimCenterXYPx
    %     min([mean(p.trial.fixTrain.stimSizeXYPx) 189])
    %     p.trial.fixTrain.stimColorRGB
    %     p.trial.display.screenCenterPx
    %     strcmp(p.trial.fixTrain.stimType,'circle')
    
    % dbg line
    %Screen('DrawDots',p.trial.display.overlayptr,p.trial.fixTrain.stimCenterXYPx,min([mean(p.trial.fixTrain.stimSizeXYPx) 189])); Screen('Flip',p.trial.display.overlayptr)
    % no idea why i have to do this but ok
    
    Screen('DrawDots',p.trial.display.overlayptr,...
        p.trial.fixTrain.stimCenterXYPx, ...
        min([mean(p.trial.fixTrain.stimSizeXYPx) 189]), ...
        p.trial.fixTrain.stimColorRGB, ...
        p.trial.display.screenCenterPx, ...
        2);
elseif strcmp(p.trial.fixTrain.stimType,'square')
    Screen('DrawDots',p.trial.display.overlayptr,...
        p.trial.fixTrain.stimCenterXYPx, ...
        min([mean(p.trial.fixTrain.stimSizeXYPx) 189]), ...
        p.trial.fixTrain.stimColorRGB, ...
    ...
    disp('that stimulus shape not yet implemented! pausing')
    pause
end
%[minSmoothPointSize, maxSmoothPointSize, minAliasedPointSize,
%maxAliasedPointSize] = Screen('DrawDots', windowPtr, xy [,size]
%[,color] [,center] [,dot_type][, lenient]);

%%% draw the fixation window on the experimenter pc only
...
if p.trial.pldaps.draw.framerate.use && p.trial.iFrame>2
    ...
    Screen('DrawText',p.trial.display.overlayptr, ...
        ['trial=' num2str(p.trial.pldaps.iTrial) ...
        'epoch=' num2str(p.trial.fixTrain.epoch)], ...
        200,200,[0 255 0]);
    % ['FPS=' num2str(1/p.trial.pldaps.draw.framerate.data(end))], ...
    % 200,200,[0 255 0]);
    %plot
...
if p.trial.pldaps.draw.eyepos.use
    ...
    % 0,0 voltage is top L of Screen
    % so need offset
    Screen('Drawdots',  p.trial.display.overlayptr, ...
...
% end of function frameDrawingFinished
...
p.trial.framePreLastDrawIdleCount=0; % amb 11/28/16 is this used??
p.trial.framePostLastDrawIdleCount=0; % amb 11/28/16 is this used??
end %frameFlip
...
%% check if start of new trial, if so, start time and switch state
if ~p.trial.fixTrain.epoch
    % start a timer
    p.trial.fixTrain.preStimTOn=GetSecs-p.trial.trstart;
    % change to state 1:preStimEpoch
    p.trial.fixTrain.epoch=1;
    disp([datestr(now,'HH:MM:SS.FFF') ...
        ' trial is ' num2str(p.trial.pldaps.iTrial)])
end
