...
%Set up the CLUTs
defaultColor = [1, 0, 1]; 
clutTestDisplay = repmat(defaultColor, [256,1]);   % By default, all overlays are Magenta
clutConsoleDisplay = repmat(defaultColor, [256,1]);   % By default, all overlays are Magenta

clutTestDisplay(242:246,:) = repmat([1, 0, 0], [5,1]);   % Items drawn with 242:246 show on test display as red % FOR MAC
clutTestDisplay(252:256,:) = repmat([0, 0, 1], [5,1]);   % Items drawn with 252:256 show on test display as blue % FOR MAC

clutConsoleDisplay(242:246,:) = repmat([1, 0, 0], [5,1]);   % Items drawn with 242:246 show on console display in red
clutConsoleDisplay(247:251,:) = repmat([0, 1, 0], [5,1]);   % Items drawn with 247:251 show on console display as green % 
clutConsoleDisplay(252:256,:) = repmat([0, 0, 1], [5,1]);   % Items drawn with 252:256 show on console display as blue % FOR MAC
...
%Screen('BlendFunction', windowPtr, 'GL_ONE', 'GL_ONE');
    ...
    DrawFormattedText(windowPtr, 'Should be in red', 'center', 40, [0 244 0]);      % Overlay only visible on test display
    DrawFormattedText(windowPtr, 'Should be in green', 'center', 80, [0 250 0]);   % Overlay only visible on console display
    DrawFormattedText(windowPtr, 'Shoulder be in blue', 'center', 'center', [0 253 0]);
    % Draw borders
