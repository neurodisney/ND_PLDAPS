function R = ND_GetFixDur(minval, maxval, rndmeth, mu, n, step)
...
if (mu > maxval)
    error('Maximum FixDur tme can not exceed mean FixDur time!');
end

if (mu < minval)
    error('Minimum FixDur tme can not exceed mean FixDur time!');
end
...
%         end
        ...
        while(~isempty(pos))
            R(pos) = ND_GetFixDur(minval, maxval, rndmeth, mu+minval, length(pos), step);
            pos = find(R > maxval);
        end
