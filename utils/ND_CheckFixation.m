...
if(p.trial.mouse.useAsEyepos)
    ...
    p.trial.mouse.Amp(sIdx) = sqrt((p.trial.mouse.X(sIdx) - p.trial.behavior.fixation.FixPos(1) - p.trial.Calib.offset(1)).^2 + ...
                                   (p.trial.mouse.Y(sIdx) - p.trial.behavior.fixation.FixPos(2) - p.trial.Calib.offset(2)).^2);
    ...
    % calculate a moving average of the eye position for display reasons
    p.trial.eyeX   = p.trial.Calib.gain(1) * (prctile(p.trial.AI.Eye.X(sIdx), 50) - p.trial.Calib.offset(1));
    p.trial.eyeY   = p.trial.Calib.gain(2) * (prctile(p.trial.AI.Eye.Y(sIdx), 50) - p.trial.Calib.offset(2));
